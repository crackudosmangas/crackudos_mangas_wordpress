<?php
	...
	class WidgetExtension {
		public function __construct() {
			...
			add_action( 'sidebar_admin_setup', array( $this, 'add_widget_custom_variation_field' ), 1 );
		...
		public function before_widget( $params ) {
			...
			if ( $params[0]['before_widget'] != '' ) {
				...
				if ( ! $heading_style || $heading_style == '' || $heading_style == 'heading-style-1' ) {
					$params[0]['before_title'] = apply_filters( 'madara_sidebar_before_title_style_1', '<div class="widget-heading font-nav"><h5 class="heading">' );
					$params[0]['after_title']  = apply_filters( 'madara_sidebar_after_title_style_1', '</h5></div>' );
				} else {
					$params[0]['before_title'] =  apply_filters( 'madara_sidebar_before_title_style_2', '<div class="c-blog__heading style-2 font-heading"><h4>');
					$params[0]['after_title']  = apply_filters( 'madara_sidebar_after_title_style_2', '</h4></div>');
				}
				...
				$w_options = isset($w_options[ $w_index ]) ? $w_options[ $w_index ] : array();
